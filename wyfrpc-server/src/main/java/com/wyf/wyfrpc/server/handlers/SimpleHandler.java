...
public class SimpleHandler extends SimpleChannelInboundHandler<String> {
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, String msg) throws Exception {
        ...
        if(Const.METHOD_PING.equals(method)){
            ...
            // 调用
            if(0 == rpcDTO.getCode()){
                String to = rpcDTO.getTo();
                if(!Const.CHANNEL_MAP.containsKey(to)){
                    // 被调用方不在线
                    rpcDTO.setCode(5);
                    // 返回 调用方
                    ...
                    rpcDTO.setCode(1);
                    // 转发给 被调用方
                    SendUtil.send(Const.CHANNEL_MAP.get(to),rpcDTO);
                }
                // 返回
            }else if(2 == rpcDTO.getCode()){
                String from  = rpcDTO.getFrom();
                if(!Const.CHANNEL_MAP.containsKey(from)){
                    // 调用方不在线
                    log.info("调用方不在线");
